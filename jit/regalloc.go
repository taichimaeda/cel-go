...
// Location is the final assigned location for a VReg.
...
func (a *allocator) nextSpillLocation(t Type) Location {
	loc := Location{IsSpill: true, Reg: -1, Reg2: -1, Slot: a.nextSpill}
	a.nextSpill++
	if t == T_STRING {
		a.nextSpill++
	}
...
func (a *allocator) validateAlloc() error {
	for vreg, loc := range a.alloc {
		if loc.IsSpill || loc.Reg < 0 {
			return fmt.Errorf("%w: vreg v%d has invalid primary location (spill=%t reg=%d)", ErrCodegenUnsupported, vreg, loc.IsSpill, loc.Reg)
		}
		if loc.Reg < 100 {
