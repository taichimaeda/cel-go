...
func Translate(ast *celast.AST, activationType reflect.Type) (*Program, error) {
	...
	fieldMap, err := tr.buildStructFieldMap()
	if err != nil {
		return nil, err
	}
...
func (t *translator) translateLiteral(v ref.Val) (VReg, Type, error) {
	...
	case bool:
		t.emitInstr(Instr{Op: CONST_BOOL, Dst: dst, Imm: int64(boolToInt(c)), Type: T_BOOL})
...
func (t *translator) translateIdent(expr celast.Expr) (VReg, Type, error) {
	// Resolve constant reference from the reference map
	idRef, found := t.ast.ReferenceMap()[expr.ID()]
	if found && idRef != nil && idRef.Value != nil {
		return t.translateLiteral(idRef.Value)
	}

	// Resolve environment variable reference by looking up struct field
	idName := expr.AsIdent()
...
func (t *translator) translateCall(expr celast.Expr) (VReg, Type, error) {
	...
	// Dispatch special operators that require custom control flow
	switch fn {
	...
	// Collect all arguments including receiver for member functions
	...
	// Translate all arguments to virtual registers
	...
	// Resolve the overload to get the opcode/builtin and result type
	...
	spec, found := t.resolveOverload(overloads[0], argTypes)
	if !found {
...
func (t *translator) translateLogicalAnd(args []celast.Expr) (VReg, Type, error) {
	...
	falseLbl := t.newLabel()
	endLbl := t.newLabel()
	// Jump to false label if any argument is false (short circuit)
	for _, arg := range args {
		r, typ, err := t.translateExpr(arg)
		if err != nil {
			return 0, T_UNSPECIFIED, err
	...
	// At least one argument was false
...
func (t *translator) translateLogicalOr(args []celast.Expr) (VReg, Type, error) {
	...
	trueLbl := t.newLabel()
	endLbl := t.newLabel()
	// Jump to true label if any argument is true (short circuit)
	for _, arg := range args {
		r, typ, err := t.translateExpr(arg)
		if err != nil {
			return 0, T_UNSPECIFIED, err
	...
	t.emitInstr(Instr{Op: CONST_BOOL, Dst: dst, Imm: 0, Type: T_BOOL})
	t.emitInstr(Instr{Op: BR, Lbl: endLbl})
	// At least one argument was true
	t.emitLabel(trueLbl)
	t.emitInstr(Instr{Op: CONST_BOOL, Dst: dst, Imm: 1, Type: T_BOOL})
	t.emitLabel(endLbl)
	return dst, T_BOOL, nil
...
func (t *translator) translateConditional(args []celast.Expr) (VReg, Type, error) {
	...
	// Evaluate then branch (condition was true)
	...
	// Evaluate else branch (condition was false)
	t.emitLabel(elseLbl)
...
func (t *translator) translateInList(expr celast.Expr, args []celast.Expr) (VReg, Type, error) {
	...
	// Check if needle type is comparable
	switch needleType {
	case T_INT64, T_UINT64, T_FLOAT64, T_BOOL, T_STRING:
	default:
		return 0, T_UNSPECIFIED, fmt.Errorf("%w: in_list needle type %v is not supported", ErrTranslateUnsupported, needleType)
	}
	switch rhs := args[1]; rhs.Kind() {
	case celast.ListKind:
		return t.translateInListLiteral(needle, needleType, rhs.AsList().Elements())
	case celast.IdentKind:
		return t.translateInListIdent(needle, needleType, rhs.AsIdent())
	default:
		return 0, T_UNSPECIFIED, fmt.Errorf("%w: in_list RHS kind %v is unsupported", ErrTranslateUnsupported, rhs.Kind())
	}
}

func (t *translator) translateInListLiteral(needle VReg, needleType Type, elems []celast.Expr) (VReg, Type, error) {
	const maxElems = 1024
	...
	for _, elem := range elems {
		...
		lit := t.newVReg()
		// Emit constant load and comparison based on needle type
		switch needleType {
		case T_STRING:
			s, ok := elem.AsLiteral().Value().(string)
			if !ok {
				return 0, T_UNSPECIFIED, fmt.Errorf("%w: in_list literal elements must match needle type string", ErrTranslateUnsupported)
			}
			t.emitInstr(Instr{
				Op:   CONST_STRING,
				Dst:  lit,
		...
		case T_INT64:
			i, ok := elem.AsLiteral().Value().(int64)
			if !ok {
				return 0, T_UNSPECIFIED, fmt.Errorf("%w: in_list literal elements must match needle type int64", ErrTranslateUnsupported)
			}
			t.emitInstr(Instr{
				Op:   CONST_INT,
				Dst:  lit,
			...
			t.emitInstr(Instr{Op: BR_TRUE, Src1: cmp, Lbl: matchLbl})
		case T_UINT64:
			u, ok := elem.AsLiteral().Value().(uint64)
			if !ok {
				return 0, T_UNSPECIFIED, fmt.Errorf("%w: in_list literal elements must match needle type uint64", ErrTranslateUnsupported)
			}
			t.emitInstr(Instr{
				Op:   CONST_UINT,
				Dst:  lit,
			...
			t.emitInstr(Instr{Op: BR_TRUE, Src1: cmp, Lbl: matchLbl})
		case T_FLOAT64:
			f, ok := elem.AsLiteral().Value().(float64)
			if !ok {
				return 0, T_UNSPECIFIED, fmt.Errorf("%w: in_list literal elements must match needle type float64", ErrTranslateUnsupported)
			}
			t.emitInstr(Instr{
				Op:   CONST_FLOAT,
				Dst:  lit,
				Imm:  int64(math.Float64bits(f)),
				Type: T_FLOAT64,
			})
			cmp := t.newVReg()
			t.emitInstr(Instr{
			...
			t.emitInstr(Instr{Op: BR_TRUE, Src1: cmp, Lbl: matchLbl})
		case T_BOOL:
			b, ok := elem.AsLiteral().Value().(bool)
			if !ok {
				return 0, T_UNSPECIFIED, fmt.Errorf("%w: in_list literal elements must match needle type bool", ErrTranslateUnsupported)
			}
			imm := int64(0)
			if b {
				imm = 1
			...
			t.emitInstr(Instr{Op: BR_TRUE, Src1: cmp, Lbl: matchLbl})
		default:
			return 0, T_UNSPECIFIED, fmt.Errorf("%w: in_list literal with needle type %v is unsupported", ErrTranslateUnsupported, needleType)
		}
	}
	t.emitInstr(Instr{Op: CONST_BOOL, Dst: result, Imm: 0, Type: T_BOOL})
	t.emitInstr(Instr{Op: BR, Lbl: endLbl})
...
func (t *translator) translateInListIdent(needle VReg, needleType Type, identName string) (VReg, Type, error) {
	...
	// Determine the expected element kind and builtin based on needle type
	var expectedKind reflect.Kind
	var sliceBuiltin, arrayBuiltin BuiltinID
	switch needleType {
	case T_STRING:
		expectedKind = reflect.String
		sliceBuiltin = BuiltinListContainsStringSlice
		arrayBuiltin = BuiltinListContainsStringArray
	case T_INT64:
		expectedKind = reflect.Int64
		sliceBuiltin = BuiltinListContainsIntSlice
		arrayBuiltin = BuiltinListContainsIntArray
	case T_UINT64:
		expectedKind = reflect.Uint64
		sliceBuiltin = BuiltinListContainsUintSlice
		arrayBuiltin = BuiltinListContainsUintArray
	case T_FLOAT64:
		expectedKind = reflect.Float64
		sliceBuiltin = BuiltinListContainsFloatSlice
		arrayBuiltin = BuiltinListContainsFloatArray
	case T_BOOL:
		expectedKind = reflect.Bool
		sliceBuiltin = BuiltinListContainsBoolSlice
		arrayBuiltin = BuiltinListContainsBoolArray
	default:
		return 0, T_UNSPECIFIED, fmt.Errorf("%w: in_list with needle type %v is unsupported", ErrTranslateUnsupported, needleType)
	}

	ptr := t.newVReg()
	switch field.Type.Kind() {
	case reflect.Slice:
		if field.Type.Elem().Kind() != expectedKind {
			return 0, T_UNSPECIFIED, fmt.Errorf("%w: list field %q element type must be %v to match needle type, got %v", ErrTranslateUnsupported, identName, expectedKind, field.Type.Elem().Kind())
		}
		t.emitInstr(Instr{
			Op:   LOAD_FIELD_SLICE,
			Dst:  ptr,
...
func (t *translator) buildStructFieldMap() (map[string][]int, error) {
	fields := make(map[string][]int)
	collisions := make(map[string]bool)

	addField := func(name string, index []int) {
		if name == "" {
			return
		}
		if idx, found := fields[name]; found && !slices.Equal(idx, index) {
	...
	for _, field := range reflect.VisibleFields(t.structType) {
		...
		// Add the exact Go field name (e.g., "UserID")
		addField(field.Name, field.Index)
		// Add JSON tag name if present (e.g., json:"user_id" -> "user_id")
		...
		// Add protobuf tag name if present (e.g., protobuf:"...,name=user_id,..." -> "user_id")
		...
		// Add lower case field name (e.g., "UserID" -> "userID")
...
func (t *translator) resolveOverload(overload string, args []Type) (overloadSpec, bool) {
	if spec, found := overloadTable[overload]; found {
		return spec, true
	}
	// Fall back to generic overload resolution
	return t.resolveGenericOverload(overload, args)
}
